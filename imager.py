    ...
    for chunkX in range(origin[0] // 16, math.ceil((size[0] + origin[0])/ 16)):
        ...
        for chunkZ in range(origin[1] // 16 - 1, math.ceil((size[1] + origin[1])/ 16)):
            if (chunkX, chunkZ) not in chunks:
                ...
                continue
            progress += 1
            print(f" Working on chunk {progress} of ~{(size[0] // 16 + 1) * (size[1] // 16 + 1)}{status}", end='\r')
            ...
            if mode[:2] == 'y=':
                ...
                heights = decode_h_map(chunk, 'WORLD_SURFACE')
                ocean_heights = decode_h_map(chunk, 'OCEAN_FLOOR')
            ...
            for i in range(256):
                ...
                y = heights[x][z]
                if mode == 'minecraft' or mode[:2] == 'y=':
                    color = get_color_from_block(blocks, heights, prev_heights, ocean_heights, x, y, z)
...
def decode_h_map(chunk, map_name):
    ...
    raw_map = decoder.longs_to_int(chunk['Heightmaps'][map_name])
    for i in range(256):
...
def get_color_from_block(blockstates, heights, northern_heights, ocean_heights, x, y, z):
    mapped_color = color_map.setdefault(str(blockstates[x][y][z]), 0)
    ...
    if mapped_color == 12:
        return get_water_color(blockstates, heights, ocean_heights, x, y, z)
...
def get_water_color(blockstates, heights, ocean_heights, x, y, z):
    depth = y - ocean_heights[x][z]
    if y != heights[x][z]:
        surface_y = y
        while color_map.setdefault(str(blockstates[x][y][z]), 0) in (1, 12):
            y -= 1
        depth = surface_y - y
